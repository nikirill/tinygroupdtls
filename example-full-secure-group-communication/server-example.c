...
#define TA_KEY		     "TheTAKeyHaveToBeMoreThan32bytesLongSoImWritingThisHereToMakeItAsLongAsPossible"
#define TA_KEY_LEN	     78
...
get_local_ip(void)
{
  ...
  PRINTF("Cannot find a local address\r\n");
  return 0;
}
...
void secure_group_creation(const session_t *mcast_session) {
  ...
  /** making a ServerWrite key individual */
  dtls_hmac_keys(group, get_local_ip(), LOCAL_PORT_SECURE);
...
#if defined(WITH_DTLS) && defined(WITH_MULTICAST)
  session_t session;
...
#endif // WITH_DTLS && WITH_MULTICAST
